  ...
  return superagent
    ...
    .then(ids => {
      ...
      return campsiteArray
        .map(id =>({ campsiteId: id, availabilities: Object.entries(ids.campsites[id].availabilities) }));
    })
    .then(res => (res
      .map(campsite => ({ campsiteId: campsite.campsiteId, availabilities: campsite.availabilities
        .filter(([date, isAvailable]) => isAvailable === 'Available') }))))
